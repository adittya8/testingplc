  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="individualCommandModalLabel">
          {{ __('texts.individual_schedule') }}</h1>
        ...
        <div class="row">
          ...
          <div class="col-12 dt-hide-search">
            <table class="table text-center" id="individualLumTable">
              ...
              <tbody>
                @if (config('luminaries') && count(config('luminaries')))
                  @foreach (config('luminaries') as $lum)
                    <tr wire:key="{{ $lum->id }}">
                      <td>{{ $lum->node_id }}</td>
                      <td>{{ $lum->concentrator->concentrator_no }}</td>
                      <td>
                        <a href="#" class="link-secondary"
                          onclick="openDim({{ $lum->id }}, 'Dimming {{ $lum->node_id }}', 'individual')">
